...
import { 
  ...
  ChevronDown, ChevronUp, Printer, Phone, Mail, ShieldCheck, Shield, Check, Lock as LockIcon,
...
export const PassengerPanel: React.FC<PassengerPanelProps> = ({ initialTracking }) => {
  ...
  const scrollExpanded = (tripId: string) => {
    setTimeout(() => {
      const el = document.getElementById(`expanded-${tripId}`);
      if (el) {
         const yOffset = -20; 
  ...
  return (
                              ...
                              <AnimatePresence mode="wait">
                                {bookingStep === 1 && (
                                  <motion.div 
                                    ...
                                    animate={{ opacity: 1, x: 0 }}
                                    exit={{ opacity: 0, x: -20 }}
                                    className="space-y-4 pb-32 lg:pb-0"
                                  >
                                    {/* Legend */}
                                        ...
                                        <div className="bg-white p-5 sm:p-8 rounded-3xl border border-slate-50 shadow-sm space-y-6 sm:space-y-8">
                                          ...
                                          {selectedSeats.length > 0 ? (
                                            <div className="space-y-6 sm:space-y-8">
                                              ...
                                              <button 
                                                onClick={() => { setBookingStep(2); scrollExpanded(trip.id); }}
                                ...
                                {bookingStep === 2 && (
                                  <motion.div 
                                    ...
                                    animate={{ opacity: 1, x: 0 }}
                                    exit={{ opacity: 0, x: -20 }}
                                    className="grid lg:grid-cols-2 gap-8 pb-32 lg:pb-0"
                                  >
                                    ...
                                    <div className="space-y-6">
                                      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-8 sticky top-24">
                                        ...
                                        <div className="flex gap-6 mt-10">
                                          <button 
                                            onClick={() => { setBookingStep(1); scrollExpanded(trip.id); }}
                                          ...
                                          <button 
                                            ...
                                            onClick={() => { setBookingStep(3); scrollExpanded(trip.id); }}
                                ...
                                {bookingStep === 3 && (
                                   <motion.div 
                                    ...
                                    animate={{ opacity: 1, x: 0 }}
                                    exit={{ opacity: 0, x: -20 }}
                                    className="max-w-2xl mx-auto w-full pb-32 lg:pb-0"
                                  >
                                      <div className="bg-white sm:rounded-[2.5rem] sm:border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden flex flex-col min-h-[100dvh] sm:min-h-0">
                                        ...
                                        <div className="sm:hidden flex items-center justify-between p-4 border-b border-slate-100 bg-white sticky top-0 z-10">
                                          <button onClick={() => { setBookingStep(1); scrollExpanded(trip.id); }} className="p-2 text-slate-400 hover:text-slate-600 bg-slate-50 rounded-full transition-colors">
                                            <ChevronLeft size={20} />
                                          </button>
                                        ...
                                        {/* Bottom Action Bar */}
                                        <div className="bg-white p-4 sm:p-6 border-t border-slate-100 flex flex-col sm:flex-row gap-3 sm:gap-4 sticky bottom-0 left-0 w-full z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] sm:shadow-none">
    ...
    <div className={activeTab === 'profile' ? 'block pt-24 min-h-[80vh]' : 'hidden'}>
      ...
      {/* Mobile Bottom Navigation Removed - handled by global App.tsx */}
    </div>
  );
};
